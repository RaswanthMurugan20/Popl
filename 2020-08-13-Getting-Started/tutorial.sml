...
val origin : real * real = (0.0, 0.0)
 ...
 stdIn:26.5-26.40 Error: pattern and expression in val dec don't agree [tycon mismatch]
  pattern:    real * real
  expression:    real * string
  in declaration:
    origin : real * real = (0.0,"0.0")
...
fun foldr f x [] = x | fodlr f x (y::ys) = f y foldr f x ys (*fold right*)
fun foldl f x [] = x | fodll f x (y::ys) = foldl f (f x y) ys (*fold left*)
